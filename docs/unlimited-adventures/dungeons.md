...
새로운 던전에서 몬스터를 스폰 시키고 싶은 경우, 아래 명령어를 입럭하세요:```/dungeons setmonsterspawn [몬스터 종류]```예시:```/dungeons setmonsterspawn zombie```

> 사용 가능한 몬스터 종류: `zombie`, `skeleton`, `spider`, `husk` & `ghost`.

#### :dragon: 새로운 던전에서 보스를 소환하는 방법

새로운 던전에서 보스를 스폰 시키고 싶은 경우, 아래 명령어를 입력하세요:```/dungeons setbossspawn [보스 종류] ```예시:```/dungeons setbossspawn dracula```
